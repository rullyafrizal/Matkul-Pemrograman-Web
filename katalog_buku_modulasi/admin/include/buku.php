    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            <table class="table table-bordered">
                ...
                $sql_jum = "SELECT `b`.`id_buku`, `b`.`judul`,
                ...
                if (!empty($katakunci_buku)) {
                    $sql_jum .= " where `b`.`judul` LIKE '%$katakunci_buku%' 
                            OR `k`.`kategori_buku` LIKE '%$katakunci_buku%'
                            OR `p`.`penerbit` LIKE '%$katakunci_buku%'";
                }
                $sql_jum .= " ORDER BY `b`.`id_buku` DESC";
                ...
                //pencarian
                $sql_k = "SELECT `b`.`id_buku`, `b`.`judul`,
                                `k`.`kategori_buku`,`p`.`penerbit` FROM `buku` `b`
                                INNER JOIN `kategori_buku` `k` ON `b`.`id_kategori_buku` =
                                `k`.`id_kategori_buku` INNER JOIN `penerbit` `p` ON
                                `b`.`id_penerbit` = `p`.`id_penerbit` ";
                ...
                $sql_k .= " ORDER BY `b`.`id_buku` DESC limit $posisi, $batas ";
                ...
                while ($data_b = mysqli_fetch_row($query_k)) {
                    ...
                    <tr>
                        ...
                        <td align="center">
                            <a href="index.php?include=edit-buku&data=<?php echo $id_buku; ?>"
                            ...
                            <a href="javascript:if(confirm('Anda yakin ingin menghapus buku <?php echo $judul; ?>?')) window.location.href = 'index.php?include=buku&aksi=hapus&data=<?php echo $id_buku; ?>&notif=hapusberhasil'"
