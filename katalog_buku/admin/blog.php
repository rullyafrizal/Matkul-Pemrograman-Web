                ...
                <div class="card-body">
                    ...
                    <table class="table table-bordered">
                       ...
                       `b` INNER JOIN `kategori_blog` `k` ON `b`.`id_kategori_blog` =`k`.`id_kategori_blog`";
                        ...
                        $sql_b .= "ORDER BY `b`.`id_blog` DESC limit $posisi, $batas ";
                        $query_b = mysqli_query($koneksi, $sql_b);
                        //hitung jumlah semua data
                        $sql_jum = "SELECT `b`.`id_blog`, `k`.`kategori_blog`,`b`.`judul`,`b`.`tanggal` FROM `blog` `b`
                                    INNER JOIN `kategori_blog` `k` ON `b`.`id_kategori_blog` =`k`.`id_kategori_blog`";
                        if (isset($_GET["katakunci"])) {
                            $katakunci_blog = $_GET["katakunci"];
                            $sql_jum .= " where `b`.`judul` LIKE '%$katakunci_blog%' OR `k`.`kategori_blog` LIKE '%$katakunci_blog%'";
                        }
                        $sql_jum .= " order by `id_blog` DESC";
                        ...
                        while ($data_b = mysqli_fetch_row($query_b)) {
                            ...
                            <tr>
                                ...
                                <td align="center">
                                    ...
                                    <a href="javascript:if(confirm('Anda yakin ingin menghapus data <?php echo $judul; ?>?')) window.location.href = 'blog.php?aksi=hapus&data=<?php echo $id_blog; ?>&notif=hapusberhasil'"
